//! Cross-validate the multi-part **multi-level** flat tiled encoder
//! (`encode_exr_multipart_tiled_mipmap`) against `exrheader` and
...
use oxideav_openexr::{
    build_box_filter_pyramid, encode_exr_multipart_tiled_mipmap,
    parse_exr_multipart_tiled_multilevel, parse_exr_tiled_multilevel, Channel, Compression,
    MipmapLevel, MultipartMipmapTiledPart, PixelType,
};
...
fn tempdir(tag: &str) -> String {
    ...
    let dir = std::env::temp_dir().join(format!(
        "oxideav-openexr-mpmip-{tag}-{nanos}-{}-{c}",
        std::process::id()
    ));
...
fn build_part(
    ...
    comp: Compression,
    tile: u32,
) -> (MultipartMipmapTiledPart, Vec<MipmapLevel>) {
...
#[test]
fn exrheader_accepts_our_multipart_mipmap_tiled_file() {
    ...
    assert!(
        out.status.success(),
        "exrheader failed on our multipart mipmap tiled file\nstdout: {stdout}\nstderr: {stderr}"
    );
    assert!(
        stdout.contains("tiledimage"),
        "exrheader output missing type='tiledimage'\nstdout: {stdout}"
    );
    // exrheader prints the level mode as "mip-map" (hyphenated). Accept
    // either spelling for forward/backward compatibility with reference
    // versions.
    assert!(
        stdout.contains("mip-map") || stdout.contains("mipmap"),
        "exrheader output should mention mipmap level mode\nstdout: {stdout}"
    );
    assert!(
...
#[test]
fn exrmultipart_separate_splits_our_multipart_mipmap_tiled() {
    ...
    // the source pyramid sample-for-sample.
    ...
    for s in &splits {
        ...
        let (want, source_pyr) = if txt.contains("alpha") {
            ("alpha", &pyr0)
        ...
        assert_eq!(decoded.levels.len(), source_pyr.len(), "{want} level count");
        for (got, src) in decoded.levels.iter().zip(source_pyr.iter()) {
            assert_eq!(got.width, src.width, "{want} L{} width", got.level_x);
            assert_eq!(got.height, src.height, "{want} L{} height", got.level_x);
            for (gp, sp) in got.planes.iter().zip(src.planes.iter()) {
                assert_eq!(
                    &gp.samples, sp,
                    "{want} L{} plane '{}'",
                    got.level_x, gp.name
...
#[test]
fn our_writer_and_reader_multipart_mipmap_full_roundtrip() {
    // Pure self-roundtrip exercising the public-API import path,
    // independent of any reference binary.
