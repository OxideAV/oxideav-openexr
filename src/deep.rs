...
pub fn parse_exr_deep_tiled(bytes: &[u8]) -> Result<DeepTiledImage> {
    ...
    let (tile_x, tile_y) = match &tile_attr.value {
        AttributeValue::Other { type_name, data } if type_name == "tiledesc" && data.len() == 9 => {
            ...
            // at the right one rather than reject outright.
            if (mode & 0x0F) == 0x01 {
                return Err(ExrError::unsupported(
...
pub fn parse_exr_deep_tiled_mipmap(bytes: &[u8]) -> Result<DeepMipmapTiledImage> {
    ...
    if bytes.len() >= 8 {
        ...
        if vf_peek.multipart {
            return Err(ExrError::unsupported(
                "multi-part deep tiled MIPMAP EXR \
                 (use parse_exr_multipart_deep_tiled_mipmap)"
                    .to_string(),
            ));
        }
...
//
// File layout (composes the round-130 single-part deep-tiled chunk shape
// with the round-124 single-part flat RIPMAP iteration order):
//
//   magic(4) | version(4 — non_image=0x800 only; single_tile NOT set)
//   header attributes (channels, chunkCount[int], compression, dataWindow,
//     displayWindow, lineOrder, maxSamplesPerPixel[int], pixelAspectRatio,
//     screenWindowCenter, screenWindowWidth,
//     tiles[tiledesc, mode=0x02 = RIPMAP_LEVELS + ROUND_DOWN],
//     type[string="deeptile"], version[int=1])
//   NUL terminator
//   tile offset table: chunkCount * u64 LE absolute byte offsets, where
//     chunkCount = sum over `(lvlx, lvly) ∈ nx × ny` of
//     ceil(cell_w / tile_x) * ceil(cell_h / tile_y).
//   tile chunks, each:
//     i32 tx | i32 ty | i32 lvlx | i32 lvly
//     u64 packed_pixel_offset_table_size
//     u64 packed_sample_data_size
...
//     packed_sample_data_bytes
//
// The RIPMAP grid is (nx × ny) where `nx = mipmap_level_count(width)`
// and `ny = mipmap_level_count(height)`. Cell `(lvlx, lvly)` has
// dimensions `(mipmap_level_dim(w, lvlx), mipmap_level_dim(h, lvly))`.
// The offset table walks the grid `lvly` outer / `lvlx` inner (matching
// the round-124 flat RIPMAP writer + the decoder's
// `compute_total_tiles` RIPMAP branch), and within each cell emits tile
// chunks INCREASING_Y row-major (ty outer, tx inner). Chunk headers
// carry the explicit `(lvlx, lvly)` pair (the two axes are independent
// for RIPMAP, unlike the MIPMAP diagonal).
//
// Per-tile pixel-offset table holds `tile_h * tile_w` cumulative-inclusive
// i32 entries, row-major within each tile's valid pixel rectangle (edge
// tiles trim to their valid extent). For NONE compression the reader also
...
/// One cell of a deep-tiled RIPMAP grid: explicit width/height plus
/// per-pixel sample counts and per-channel samples for that cell. Plane
/// `channel_samples[c]` is `samples_per_pixel.iter().sum()` long, in
/// pixel-scan (row-major) order. UINT channels store the u32 bits as f32
/// (matching the [`DeepExrImage`] convention).
pub struct DeepRipmapTiledLevelInput<'a> {
    pub width: u32,
    pub height: u32,
    /// `width * height` long.
    pub samples_per_pixel: &'a [u32],
...
/// Input descriptor for [`encode_exr_deep_tiled_ripmap`].
///
/// `grid[lvly][lvlx]` carries the deep pixels for grid cell `(lvlx,
/// lvly)`. The grid must be shaped `(ny, nx) ==
/// (mipmap_level_count(level0_h), mipmap_level_count(level0_w))` per the
/// ROUND_DOWN reduction convention, and every row must have the same
/// length. Cell `(lvlx, lvly)` must have dimensions `(mipmap_level_dim(w,
/// lvlx), mipmap_level_dim(h, lvly))`.
pub struct DeepRipmapTiledInput<'a> {
    /// Tile pixel dimensions. Both must be > 0; edge tiles store only
    /// the valid pixel rectangle.
    pub tile_x: u32,
    pub tile_y: u32,
    /// Channels in alphabetical order (sub-sampled channels are not
    /// permitted in tiled files per the spec).
    pub channels: Vec<Channel>,
...
/// One decoded cell of a deep-tiled RIPMAP grid.
#[derive(Debug, Clone, PartialEq)]
pub struct DeepTiledRipmapCell {
    pub level_x: u32,
    pub level_y: u32,
    pub width: u32,
    pub height: u32,
...
/// Single-part deep-tiled RIPMAP_LEVELS EXR returned by
/// [`parse_exr_deep_tiled_ripmap`]. `grid[lvly][lvlx]` matches the
/// encoder's input shape.
#[derive(Debug, Clone, PartialEq)]
pub struct DeepRipmapTiledImage {
    pub data_window: Box2i,
    pub display_window: Box2i,
    pub line_order: LineOrder,
    pub compression: Compression,
...
/// Encode a single-part `type="deeptile"` RIPMAP_LEVELS deep-tiled EXR.
///
/// Self-roundtrips through [`parse_exr_deep_tiled_ripmap`].
pub fn encode_exr_deep_tiled_ripmap(input: &DeepRipmapTiledInput) -> Result<Vec<u8>> {
    // ---- Validate input. ----
    if !matches!(
        input.compression,
        Compression::None | Compression::Rle | Compression::Zips
        ...
        return Err(ExrError::unsupported(format!(
            "deep ripmap tiled encode compression {:?} (reference encoder accepts \
             only NONE/RLE/ZIPS for deep — ZIP rejected by exrinfo with \
             EXR_ERR_INVALID_ATTR)",
            input.compression
        )));
    ...
    if input.tile_x == 0 || input.tile_y == 0 {
        return Err(ExrError::invalid(format!(
            "deep ripmap tile size {}×{} must both be > 0",
            input.tile_x, input.tile_y
        )));
    }
    if input.grid.is_empty() || input.grid[0].is_empty() {
        return Err(ExrError::invalid(
            "deep ripmap grid must have at least one cell".to_string(),
        ));
    }
    for ch in &input.channels {
        if ch.x_sampling != 1 || ch.y_sampling != 1 {
            return Err(ExrError::unsupported(format!(
                "deep ripmap tiled encode + sub-sampled channel '{}' \
                 (spec requires 1×1 in tiled files)",
                ch.name
            )));
        }
    ...
    for win in input.channels.windows(2) {
        if win[0].name >= win[1].name {
            return Err(ExrError::invalid(format!(
                "deep ripmap tiled channels not alphabetical: '{}' >= '{}'",
                win[0].name, win[1].name
            )));
        }
    }

    // Level-(0,0) carries the full-resolution dimensions used in the
    // dataWindow.
    let width = input.grid[0][0].width;
    let height = input.grid[0][0].height;
    if width == 0 || height == 0 {
        return Err(ExrError::invalid(format!(
            "deep ripmap tiled level-(0,0) {width}×{height} must be > 0"
        )));
    }

    let nx_expected = crate::decoder::mipmap_level_count(width, false);
    let ny_expected = crate::decoder::mipmap_level_count(height, false);
    if input.grid.len() as u32 != ny_expected {
        return Err(ExrError::invalid(format!(
            "deep ripmap grid has {} rows (y-levels), expected {ny_expected} for height={height} ROUND_DOWN",
            input.grid.len()
        )));
    }
    for (lvly, row) in input.grid.iter().enumerate() {
        if row.len() as u32 != nx_expected {
            return Err(ExrError::invalid(format!(
                "deep ripmap grid row lvly={lvly} has {} cells (x-levels), expected {nx_expected} for width={width} ROUND_DOWN",
                row.len()
            )));
        }
    }
    ...
    for (lvly, row) in input.grid.iter().enumerate() {
        ...
        for (lvlx, cell) in row.iter().enumerate() {
            ...
            if cell.width != want_w || cell.height != want_h {
                return Err(ExrError::invalid(format!(
                    "deep ripmap cell (lvlx={lvlx}, lvly={lvly}) is {}×{} but spec requires {want_w}×{want_h} (ROUND_DOWN)",
            ...
            if cell.samples_per_pixel.len() != need_pixels {
                return Err(ExrError::invalid(format!(
                    "deep ripmap cell (lvlx={lvlx}, lvly={lvly}) samples_per_pixel len {} != {}*{} = {need_pixels}",
                    cell.samples_per_pixel.len(),
                    cell.width,
                    cell.height
                )));
            }
            if cell.channel_samples.len() != input.channels.len() {
                return Err(ExrError::invalid(format!(
                    "deep ripmap cell (lvlx={lvlx}, lvly={lvly}) has {} channel slices but {} channels declared",
                    cell.channel_samples.len(),
                    input.channels.len()
                )));
            }
            let cell_total: u64 = cell.samples_per_pixel.iter().map(|&n| n as u64).sum();
            for (ch, slc) in input.channels.iter().zip(cell.channel_samples.iter()) {
                if slc.len() != cell_total as usize {
                    return Err(ExrError::invalid(format!(
                        "deep ripmap cell (lvlx={lvlx}, lvly={lvly}) channel '{}' sample slice len {} != cell total {cell_total}",
                        ch.name,
                        slc.len()
                    )));
                }
    ...
    // chunkCount = sum of tile-grid size per cell.
    let mut chunk_count: usize = 0;
    for row in &input.grid {
        for cell in row {
            let tx = cell.width.div_ceil(input.tile_x);
            let ty = cell.height.div_ceil(input.tile_y);
            chunk_count += (tx as usize) * (ty as usize);
        }
    }

    // maxSamplesPerPixel across all cells.
    let max_samples = input
        .grid
        .iter()
    ...
    // ---- Build per-tile chunk payloads in (lvly, lvlx, ty, tx) order. ----
    struct ChunkBlob {
        tx: u32,
        ty: u32,
        lvlx: u32,
    ...
    for (lvly_idx, row) in input.grid.iter().enumerate() {
        for (lvlx_idx, cell) in row.iter().enumerate() {
            let lvlx_u = lvlx_idx as u32;
            let lvly_u = lvly_idx as u32;
            let lw = cell.width as usize;
            let tx_count = cell.width.div_ceil(input.tile_x);
            let ty_count = cell.height.div_ceil(input.tile_y);
            // Pre-compute per-pixel sample starts so we can slice into
            // channel slices by pixel index.
            let pixel_sample_starts: Vec<u64> = {
            ...
            for ty in 0..ty_count {
                for tx in 0..tx_count {
                    let x0 = tx * input.tile_x;
                    let y0 = ty * input.tile_y;
                    let x1 = (x0 + input.tile_x).min(cell.width);
                    let y1 = (y0 + input.tile_y).min(cell.height);
                    let tw = (x1 - x0) as usize;
                    let th = (y1 - y0) as usize;
                    let entries = tw * th;
                    ...
                    for r in 0..th {
                        ...
                        for c in 0..tw {
                            ...
                            row_acc = row_acc.checked_add(n as i32).ok_or_else(|| {
                                ExrError::invalid(format!(
                                    "deep ripmap tile (lvlx={lvlx_idx}, lvly={lvly_idx}, \
                                     tx={tx}, ty={ty}) row {r}: cumulative offset overflows i32"
                                ))
                            })?;
                            table_bytes.extend_from_slice(&row_acc.to_le_bytes());
                        }
...
/// Parse a single-part `type="deeptile"` RIPMAP_LEVELS deep-tiled EXR
/// back into a [`DeepRipmapTiledImage`].
///
/// Compression: NONE / RLE / ZIPS. The reader uses a linear chunk scan
/// (matching the round-130 single-part deep-tiled ONE_LEVEL reader, the
...
pub fn parse_exr_deep_tiled_ripmap(bytes: &[u8]) -> Result<DeepRipmapTiledImage> {
    ...
    if !header.version.non_image {
        return Err(ExrError::invalid(
            "parse_exr_deep_tiled_ripmap called on a flat (non-deep) file".to_string(),
        ));
    }
    let part_type = find_string_attr(&header.attributes, "type").ok_or_else(|| {
        ExrError::invalid("deep ripmap tiled file missing required 'type' attribute".to_string())
    })?;
    if part_type != "deeptile" {
        return Err(ExrError::unsupported(format!(
            "deep ripmap tiled file type='{part_type}' (only 'deeptile' supported)"
        )));
    }
    let chunk_count = find_int_attr(&header.attributes, "chunkCount").ok_or_else(|| {
        ExrError::invalid(
            "deep ripmap tiled file missing required 'chunkCount' attribute".to_string(),
        )
    })? as usize;
    let data_window = find_box2i(&header.attributes, "dataWindow").ok_or_else(|| {
        ExrError::invalid(
            "deep ripmap tiled file missing required 'dataWindow' attribute".to_string(),
        )
    })?;
    let display_window = find_box2i(&header.attributes, "displayWindow").unwrap_or(data_window);
    let line_order =
    ...
    let compression = find_compression(&header.attributes, "compression").ok_or_else(|| {
        ExrError::invalid(
            "deep ripmap tiled file missing required 'compression' attribute".to_string(),
        )
    })?;
    if !matches!(
        compression,
        ...
        return Err(ExrError::invalid(format!(
            "deep ripmap tiled file uses compression {compression:?} \
             (only NONE/RLE/ZIPS accepted for deep)"
        )));
    }
    let channels = find_channels(&header.attributes).ok_or_else(|| {
        ExrError::invalid(
            "deep ripmap tiled file missing required 'channels' attribute".to_string(),
        )
    })?;
    let mut sorted_channels = channels.clone();
    sorted_channels.sort_by(|a, b| a.name.cmp(&b.name));
    for ch in &sorted_channels {
        if ch.x_sampling != 1 || ch.y_sampling != 1 {
            return Err(ExrError::unsupported(format!(
                "deep ripmap tiled + sub-sampled channel '{}' \
                 (spec requires 1×1 in tiled files)",
                ch.name
            )));
        }
    ...
    let (tile_x, tile_y) = match &tile_attr.value {
        AttributeValue::Other { type_name, data } if type_name == "tiledesc" && data.len() == 9 => {
            ...
            if xs == 0 || ys == 0 {
                return Err(ExrError::invalid(format!(
                    "deep ripmap tiled tile size {xs}×{ys} must both be > 0"
                )));
            }
            (xs, ys)
        }
    ...
    if width == 0 || height == 0 {
        return Err(ExrError::invalid(format!(
            "deep ripmap tiled dataWindow {width}×{height} must be > 0"
        )));
    }

    // Compute the expected ripmap grid (sizes + chunk count) and
    // pre-allocate level data structures so we can scatter incoming tiles
    // by (lvlx, lvly, tx, ty) regardless of arrival order.
    ...
    if chunk_count != expected_chunks {
        return Err(ExrError::invalid(format!(
            "deep ripmap tiled chunkCount={chunk_count} disagrees with grid \
             total {expected_chunks} ({nx}×{ny} cells, tile {tile_x}×{tile_y})"
        )));
    }
    ...
    if pos + chunk_count * 8 > bytes.len() {
        return Err(ExrError::invalid(
            "deep ripmap tiled offset table runs past EOF".to_string(),
        ));
    }
    let mut offsets = Vec::with_capacity(chunk_count);
    for _ in 0..chunk_count {
    ...
    // For each grid cell, pre-allocate `samples_per_pixel`; we scatter
    // tiles into it as we decode.
    let mut cell_spp: Vec<Vec<Vec<u32>>> = metas
        .iter()
        .map(|row| {
            row.iter()
                .map(|m| vec![0u32; (m.width as usize) * (m.height as usize)])
    ...
    // Per-cell, per-tile decoded payloads; will be reassembled into
    // pixel-major order after the scan.
    struct TileDecoded {
        tx: u32,
        ty: u32,
    ...
    let mut cell_tiles: Vec<Vec<Vec<Option<TileDecoded>>>> = metas
        .iter()
        .map(|row| {
            row.iter()
                .map(|m| {
    ...
    for (chunk_idx, &block_off) in offsets.iter().enumerate() {
        ...
        let lvlx = i32::from_le_bytes(bytes[block_off + 8..block_off + 12].try_into().unwrap());
        let lvly = i32::from_le_bytes(bytes[block_off + 12..block_off + 16].try_into().unwrap());
        if lvlx < 0 || (lvlx as u32) >= nx {
            return Err(ExrError::invalid(format!(
                "deep ripmap tile chunk {chunk_idx}: lvlx={lvlx} outside [0, {nx})"
            )));
        }
        if lvly < 0 || (lvly as u32) >= ny {
            return Err(ExrError::invalid(format!(
                "deep ripmap tile chunk {chunk_idx}: lvly={lvly} outside [0, {ny})"
            )));
        }
        let lvlx_u = lvlx as u32;
        let lvly_u = lvly as u32;
        let meta = &metas[lvly_u as usize][lvlx_u as usize];
        if tx < 0 || ty < 0 || (tx as u32) >= meta.tx_count || (ty as u32) >= meta.ty_count {
            return Err(ExrError::invalid(format!(
                "deep ripmap tile chunk {chunk_idx}: tx={tx} ty={ty} outside \
        ...
        if cumulative_flat.len() != unpacked_table_size / 4 {
            return Err(ExrError::invalid(format!(
                "deep ripmap tile chunk {chunk_idx}: offset-table size mismatch \
                 ({} != {})",
                cumulative_flat.len(),
                unpacked_table_size / 4
            )));
        ...
        let mut tile_total_samples: u64 = 0;
        let spp_buf = &mut cell_spp[lvly_u as usize][lvlx_u as usize];
        let lw = meta.width as usize;
        for r in 0..th as usize {
            let row_base = r * row_stride;
            let row_slice = &cumulative_flat[row_base..row_base + tw as usize];
        ...
        if expected_unpacked != unpacked_data {
            return Err(ExrError::invalid(format!(
                "deep ripmap tile chunk {chunk_idx}: derived unpacked_data={expected_unpacked} \
                 disagrees with header unpacked_data={unpacked_data}"
            )));
        }
        let sample_bytes =
        ...
        for (ch_idx, ch) in sorted_channels.iter().enumerate() {
            ...
            if p + need > sample_bytes.len() {
                return Err(ExrError::invalid(format!(
                    "deep ripmap tile chunk {chunk_idx}: channel {} bytes past payload end",
                    ch.name
                )));
            }
            for s in 0..(tile_total_samples as usize) {
        ...
        if p != sample_bytes.len() {
            return Err(ExrError::invalid(format!(
                "deep ripmap tile chunk {chunk_idx}: consumed {p} of {} payload bytes",
                sample_bytes.len()
            )));
        }

        let grid_idx = (ty_u as usize) * (meta.tx_count as usize) + tx_u as usize;
        if cell_tiles[lvly_u as usize][lvlx_u as usize][grid_idx].is_some() {
            return Err(ExrError::invalid(format!(
                "deep ripmap tile (lvlx={lvlx_u}, lvly={lvly_u}, tx={tx_u}, ty={ty_u}) \
                 appears more than once in the offset table"
            )));
        }
        cell_tiles[lvly_u as usize][lvlx_u as usize][grid_idx] = Some(TileDecoded {
            tx: tx_u,
            ty: ty_u,
            tw,
            th,
    ...
    // ---- Reassemble each cell's per-channel samples in pixel-scan order. ----
    let mut grid: Vec<Vec<DeepTiledRipmapCell>> = Vec::with_capacity(ny as usize);
    for lvly in 0..ny as usize {
        let mut row_out: Vec<DeepTiledRipmapCell> = Vec::with_capacity(nx as usize);
        for lvlx in 0..nx as usize {
            let meta = &metas[lvly][lvlx];
            let spp = std::mem::take(&mut cell_spp[lvly][lvlx]);
            let total_samples: u64 = spp.iter().map(|&n| n as u64).sum();
            let mut channel_samples: Vec<Vec<f32>> = (0..sorted_channels.len())
                .map(|_| Vec::with_capacity(total_samples as usize))
                .collect();
            ...
            let mut tile_starts: Vec<TileStarts> = Vec::with_capacity(cell_tiles[lvly][lvlx].len());
            for td_opt in &cell_tiles[lvly][lvlx] {
                let td = td_opt.as_ref().ok_or_else(|| {
                    ExrError::invalid(format!(
                        "deep ripmap tiled cell (lvlx={lvlx}, lvly={lvly}) grid is missing one or more tiles"
                    ))
                })?;
                let entries = (td.tw * td.th) as usize;
                let mut starts = Vec::with_capacity(entries + 1);
            ...
            for y in 0..meta.height as usize {
                ...
                for x in 0..meta.width as usize {
                    ...
                    let x_in_tile = x - (tx as usize) * tile_x as usize;
                    let grid_idx = (ty as usize) * (meta.tx_count as usize) + tx as usize;
                    let td = cell_tiles[lvly][lvlx][grid_idx].as_ref().unwrap();
                    let pixel_within_tile = y_in_tile * td.tw as usize + x_in_tile;
                    let s_start = tile_starts[grid_idx].pixel_starts[pixel_within_tile] as usize;
                    let s_end = tile_starts[grid_idx].pixel_starts[pixel_within_tile + 1] as usize;
                    for (ch_idx, dst) in channel_samples
...
// part_number prefix in the per-chunk header) with the round-208
// single-part deep-tiled MIPMAP iteration order (per-part pyramid:
// levels 0..N-1, INCREASING_Y row-major within each level, lvlx == lvly
// == level for the MIPMAP diagonal convention from the OpenEXR
// Technical Introduction).
//
// File layout:
//
//   magic(4) | version(4 — non_image=0x800 + multipart=0x1000;
//                       single_tile NOT set — type="deeptile" + tiles
//                       attribute carry the tile-ness signal)
//   per-part header attributes (channels, chunkCount[int], compression,
//     dataWindow, displayWindow, lineOrder, maxSamplesPerPixel[int],
//     name[string], pixelAspectRatio, screenWindowCenter,
//     screenWindowWidth,
//     tiles[tiledesc, mode=0x01 = MIPMAP_LEVELS + ROUND_DOWN],
//     type[string="deeptile"], version[int=1])
//     NUL terminator
//   double-NUL terminator after the last part header
//   concatenated offset tables (part 0 table, part 1 table, …), each
//     `part.chunkCount` u64 LE absolute byte offsets — robust readers
//     are expected to also tolerate zero-filled offset tables produced
//     by `exrmultipart -combine`, so this parser uses a linear scan.
//   tile chunks, each prefixed by `i32 part_number`:
//     i32 part_number
//     i32 tx | i32 ty | i32 lvlx | i32 lvly
//     u64 packed_pixel_offset_table_size
//     u64 packed_sample_data_size
...
// → 4 + (4 + 4 + 4 + 4) + 24 = 44 bytes of header per tile chunk.
//
// Per-part chunkCount = sum over levels 0..N-1 of
//     ceil(level_w / tile_x) * ceil(level_h / tile_y).
//
// Per-tile pixel-offset table holds `tile_h * tile_w` cumulative-
// inclusive i32 entries, row-major within each tile's valid pixel
// rectangle (edge tiles trim to their valid extent). For NONE
// compression the reader also accepts files that pad to the full
// `tile_x * tile_y * 4` bytes per the round-130 single-part deep-tiled
// discipline.
//
// Sample data is non-interleaved (channel-major within each tile).
// Compression NONE / RLE / ZIPS per part (deep ZIP rejected to match
// the reference `exrinfo` validator and the round-130 / round-181 /
// round-208 deep-tiled writers).
//
// ROUND_DOWN only. Multi-part deep tiled RIPMAP_LEVELS is a followup.
// ---------------------------------------------------------------------

/// One part of a multi-part deep-tiled MIPMAP_LEVELS file, for input to
/// [`encode_exr_multipart_deep_tiled_mipmap`].
///
/// `name` is mandatory and must be unique across all parts. `pyramid`
/// is the full ROUND_DOWN mipmap pyramid; `pyramid[0]` is the
/// full-resolution level and length must equal
/// `mipmap_level_count(max(width, height), false)`.
pub struct MultipartDeepMipmapTiledPart<'a> {
    /// Part name (must be unique across all parts in the file).
    pub name: String,
    /// Tile pixel dimensions. Both must be > 0; edge tiles store only
    /// the valid pixel rectangle.
...
/// One part of a multi-part deep-tiled MIPMAP_LEVELS file, returned
/// from [`parse_exr_multipart_deep_tiled_mipmap`].
///
/// Pixel data is materialised per level into the same flat layout used
/// by [`DeepMipmapTiledImage::levels`]; the tile-grid structure is
/// fully reassembled into row-major pixel coordinates before return.
#[derive(Debug, Clone, PartialEq)]
pub struct DeepMipmapTiledPart {
    pub name: String,
    pub data_window: Box2i,
    pub display_window: Box2i,
    pub line_order: LineOrder,
...
/// Encode a multi-part `type="deeptile"` MIPMAP_LEVELS deep-tiled EXR
/// (version-field bits 0x1800).
///
/// Each part is validated independently (alphabetical channel order,
/// per-level dimensions match the ROUND_DOWN pyramid, pyramid length
/// matches `mipmap_level_count`, per-pixel sample counts, per-channel
/// sample totals, compression in {NONE, RLE, ZIPS}, unique non-empty
/// name, no sub-sampling, tile dimensions > 0). Tile chunks within each
/// part walk levels `0..N-1` ascending and within each level emit
/// `ty`-outer / `tx`-inner; part chunks are concatenated in part-order
/// downstream of the concatenated per-part offset tables.
///
/// Self-roundtrips through [`parse_exr_multipart_deep_tiled_mipmap`].
pub fn encode_exr_multipart_deep_tiled_mipmap(
    parts: &[MultipartDeepMipmapTiledPart],
) -> Result<Vec<u8>> {
    if parts.is_empty() {
        return Err(ExrError::invalid(
            "encode_exr_multipart_deep_tiled_mipmap: at least one part required".to_string(),
        ));
    }
    ...
    for (i, p) in parts.iter().enumerate() {
        if p.name.is_empty() {
            return Err(ExrError::invalid(format!(
                "deep mipmap tiled part {i}: empty name"
            )));
        }
        for (j, other) in parts.iter().enumerate() {
            if j != i && other.name == p.name {
                return Err(ExrError::invalid(format!(
                    "duplicate deep mipmap tiled part name '{}' (parts {i} and {j})",
                    p.name
                )));
            }
        }
        if !matches!(
            ...
            return Err(ExrError::unsupported(format!(
                "deep mipmap tiled part '{}' compression {:?} (reference encoder \
        ...
        if p.tile_x == 0 || p.tile_y == 0 {
            return Err(ExrError::invalid(format!(
                "deep mipmap tiled part '{}': tile size {}×{} must both be > 0",
                p.name, p.tile_x, p.tile_y
            )));
        }
        if p.pyramid.is_empty() {
            return Err(ExrError::invalid(format!(
                "deep mipmap tiled part '{}': pyramid must have at least one level",
                p.name
            )));
        }
        for ch in &p.channels {
            if ch.x_sampling != 1 || ch.y_sampling != 1 {
                return Err(ExrError::unsupported(format!(
                    "deep mipmap tiled part '{}': sub-sampled channel '{}' \
        ...
        for win in p.channels.windows(2) {
            if win[0].name >= win[1].name {
                return Err(ExrError::invalid(format!(
                    "deep mipmap tiled part '{}': channels not alphabetical: '{}' >= '{}'",
                    p.name, win[0].name, win[1].name
                )));
            }
        }
        let level0_w = p.pyramid[0].width;
        let level0_h = p.pyramid[0].height;
        if level0_w == 0 || level0_h == 0 {
            return Err(ExrError::invalid(format!(
                "deep mipmap tiled part '{}': level-0 {level0_w}×{level0_h} must be > 0",
                p.name
            )));
        }
        let expected_levels = crate::decoder::mipmap_level_count(level0_w.max(level0_h), false);
        if p.pyramid.len() as u32 != expected_levels {
            return Err(ExrError::invalid(format!(
                "deep mipmap tiled part '{}': pyramid has {} levels, expected {expected_levels} \
                 for {level0_w}×{level0_h} ROUND_DOWN",
                p.name,
                p.pyramid.len()
            )));
        }
        for (l, lvl) in p.pyramid.iter().enumerate() {
            let want_w = crate::decoder::mipmap_level_dim(level0_w, l as u32, false);
            let want_h = crate::decoder::mipmap_level_dim(level0_h, l as u32, false);
            if lvl.width != want_w || lvl.height != want_h {
                return Err(ExrError::invalid(format!(
                    "deep mipmap tiled part '{}': level {l} is {}×{} but spec requires \
            ...
            if lvl.samples_per_pixel.len() != need_pixels {
                return Err(ExrError::invalid(format!(
                    "deep mipmap tiled part '{}': level {l} samples_per_pixel len {} != {}*{} = {need_pixels}",
                    p.name,
                    lvl.samples_per_pixel.len(),
                    lvl.width,
                    lvl.height
            ...
            if lvl.channel_samples.len() != p.channels.len() {
                return Err(ExrError::invalid(format!(
                    "deep mipmap tiled part '{}': level {l} has {} channel slices but {} channels declared",
                    p.name,
                    lvl.channel_samples.len(),
                    p.channels.len()
                )));
            ...
            let level_total: u64 = lvl.samples_per_pixel.iter().map(|&n| n as u64).sum();
            for (ch, slc) in p.channels.iter().zip(lvl.channel_samples.iter()) {
                if slc.len() != level_total as usize {
                    return Err(ExrError::invalid(format!(
                        "deep mipmap tiled part '{}': level {l} channel '{}' sample slice len {} \
    ...
    for p in parts {
        ...
        for lvl in &p.pyramid {
            let tx = lvl.width.div_ceil(p.tile_x);
            let ty = lvl.height.div_ceil(p.tile_y);
    ...
    for (part_idx, p) in parts.iter().enumerate() {
        ...
        for (l, lvl) in p.pyramid.iter().enumerate() {
            let lvl_idx = l as u32;
            let lw = lvl.width as usize;
            let tx_count = lvl.width.div_ceil(p.tile_x);
            let ty_count = lvl.height.div_ceil(p.tile_y);

            // Per-level cumulative-exclusive per-pixel sample starts so
            // we can slice each channel's per-level samples by pixel
            // index.
            let pixel_sample_starts: Vec<u64> = {
                let mut v = Vec::with_capacity(lw * lvl.height as usize + 1);
                v.push(0u64);
                ...
                for tx in 0..tx_count {
                    ...
                    for r in 0..th {
                        ...
                        for c in 0..tw {
                            ...
                            row_acc = row_acc.checked_add(n as i32).ok_or_else(|| {
                                ExrError::invalid(format!(
                                    "deep mipmap tiled part '{}' tile (lvl={l}, tx={tx}, ty={ty}) \
                                     row {r}: cumulative offset overflows i32",
                                    p.name
                                ))
                            })?;
...
/// Per-part attribute set for a multipart deep-tiled MIPMAP part.
fn build_deep_mipmap_tiled_part_attrs(
...
/// Parse a multi-part `type="deeptile"` MIPMAP_LEVELS deep-tiled EXR
/// (version-field bits 0x1800) back into a `Vec<DeepMipmapTiledPart>`.
///
...
/// `mode == 0x01` (MIPMAP_LEVELS + ROUND_DOWN).
///
/// On-disk layout mirrors the single-part deep-tiled MIPMAP writer per
/// part, with the chunk record prefixed by an `i32 part_number`.
/// Compression: NONE / RLE / ZIPS only. The reader uses a linear chunk
/// scan (matching the multi-part deep-tiled ONE_LEVEL reader) for
/// robustness against zero-filled offset tables produced by
/// `exrmultipart -combine`.
pub fn parse_exr_multipart_deep_tiled_mipmap(bytes: &[u8]) -> Result<Vec<DeepMipmapTiledPart>> {
    let parts = parse_multipart_headers_allow_deep(bytes)?;
    if parts.is_empty() {
        return Err(ExrError::invalid(
            "multi-part deep tiled MIPMAP file has no parts".to_string(),
        ));
    }
    for (i, part) in parts.iter().enumerate() {
        let part_type = find_string_attr(&part.attributes, "type").ok_or_else(|| {
            ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP: part {i} missing required 'type' attribute"
            ))
        })?;
        if part_type != "deeptile" {
            return Err(ExrError::unsupported(format!(
                "multi-part deep tiled MIPMAP: part {i} type='{part_type}' \
    ...
    if !parts[0].version.non_image {
        return Err(ExrError::invalid(
            "parse_exr_multipart_deep_tiled_mipmap called on a multi-part file without \
    ...
    if !parts[0].version.multipart {
        return Err(ExrError::invalid(
            "parse_exr_multipart_deep_tiled_mipmap called on a non-multipart EXR".to_string(),
        ));
    }

    /// Per-level metadata for a part.
    struct LevelMeta {
        width: u32,
        height: u32,
        tx_count: u32,
    ...
    /// Per-tile decoded channel samples within a level.
    struct TileDecoded {
        tw: u32,
        th: u32,
        channel_samples: Vec<Vec<f32>>,
    ...
    struct PartState {
        ...
        n_levels: u32,
        metas: Vec<LevelMeta>,
        /// Per-level samples_per_pixel grid (each `width*height`).
        level_spp: Vec<Vec<u32>>,
        /// Per-level, per-tile decoded payloads, indexed by
        /// `ty * tx_count + tx`.
        level_tiles: Vec<Vec<Option<TileDecoded>>>,
    }
    ...
    for (i, part) in parts.iter().enumerate() {
        let name = find_string_attr(&part.attributes, "name").ok_or_else(|| {
            ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP part {i} missing required 'name' attribute"
            ))
        })?;
        let chunk_count = crate::decoder::find_chunk_count(&part.attributes).ok_or_else(|| {
            ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP part {i} ('{name}') missing required 'chunkCount' attribute"
            ))
        })?;
        let data_window = find_box2i(&part.attributes, "dataWindow").ok_or_else(|| {
            ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP part {i} ('{name}') missing required 'dataWindow' attribute"
            ))
        })?;
        let display_window = find_box2i(&part.attributes, "displayWindow").unwrap_or(data_window);
        let line_order =
        ...
        let compression = find_compression(&part.attributes, "compression").ok_or_else(|| {
            ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP part {i} ('{name}') missing required 'compression' attribute"
            ))
        })?;
        if !matches!(
            compression,
            ...
            return Err(ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP part {i} ('{name}') uses compression \
                 {compression:?} (only NONE/RLE/ZIPS accepted for deep)"
            )));
        }
        let channels = find_channels(&part.attributes).ok_or_else(|| {
            ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP part {i} ('{name}') missing required 'channels' attribute"
            ))
        })?;
        let mut sorted_channels = channels.clone();
        sorted_channels.sort_by(|a, b| a.name.cmp(&b.name));
        for ch in &sorted_channels {
            if ch.x_sampling != 1 || ch.y_sampling != 1 {
                return Err(ExrError::unsupported(format!(
                    "multi-part deep tiled MIPMAP part {i} ('{name}'): sub-sampled channel '{}' \
                     (tiled files require 1×1 sampling)",
                    ch.name
                )));
            }
        ...
        if width == 0 || height == 0 {
            return Err(ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP part {i} ('{name}'): dataWindow {width}×{height} \
                 must be > 0"
            )));
        }
        let n_levels = crate::decoder::mipmap_level_count(width.max(height), false);
        ...
        if chunk_count != expected_chunks {
            return Err(ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP part {i} ('{name}'): chunkCount={chunk_count} \
                 disagrees with pyramid total {expected_chunks} ({n_levels} levels, \
                 tile {tile_x}×{tile_y})"
            )));
        }
        let level_spp: Vec<Vec<u32>> = metas
            .iter()
            .map(|m| vec![0u32; (m.width as usize) * (m.height as usize)])
            .collect();
        let level_tiles: Vec<Vec<Option<TileDecoded>>> = metas
            .iter()
            .map(|m| {
                (0..(m.tx_count as usize) * (m.ty_count as usize))
                    .map(|_| None)
    ...
    if chunk_scan_start > bytes.len() {
        return Err(ExrError::invalid(format!(
            "multi-part deep tiled MIPMAP offset tables run past EOF (need {chunk_scan_start}, have {})",
            bytes.len()
        )));
    }
    ...
    for _ in 0..total_chunks {
        ...
        if scan_pos + 44 > bytes.len() {
            return Err(ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP: unexpected EOF at chunk scan position {scan_pos}"
            )));
        }
        let part_num = i32::from_le_bytes(bytes[scan_pos..scan_pos + 4].try_into().unwrap());
        let tx = i32::from_le_bytes(bytes[scan_pos + 4..scan_pos + 8].try_into().unwrap());
        ...
        if part_num < 0 || part_num as usize >= state.len() {
            return Err(ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP chunk at {scan_pos}: part_number={part_num} \
                 out of range 0..{}",
                state.len()
            )));
        }
        ...
        if lvlx != lvly {
            return Err(ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP part {part_idx} ('{}'): chunk lvlx={lvlx} != \
                 lvly={lvly} (MIPMAP diagonal requires equal levels — RIPMAP would be a \
                 separate codepath)",
                ps.name
            )));
        }
        if lvlx < 0 || (lvlx as u32) >= ps.n_levels {
            return Err(ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP part {part_idx} ('{}'): chunk lvl={lvlx} outside \
                 [0, {})",
                ps.name, ps.n_levels
            )));
        }
        let lvl = lvlx as usize;
        let meta_tx_count = ps.metas[lvl].tx_count;
        let meta_ty_count = ps.metas[lvl].ty_count;
        let meta_width = ps.metas[lvl].width;
        let meta_height = ps.metas[lvl].height;
        if tx < 0 || ty < 0 || (tx as u32) >= meta_tx_count || (ty as u32) >= meta_ty_count {
            return Err(ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP part {part_idx} ('{}'): tx={tx} ty={ty} outside \
                 level {lvl}'s grid {}×{}",
        ...
        if cumulative_flat.len() != unpacked_table_size / 4 {
            return Err(ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP part {part_idx} ('{}'): offset-table size mismatch \
                 ({} != {})",
                ps.name,
                cumulative_flat.len(),
                unpacked_table_size / 4
        ...
        let mut tile_total_samples: u64 = 0;
        let spp_buf = &mut ps.level_spp[lvl];
        ...
        if expected_unpacked != unpacked_data {
            return Err(ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP part {part_idx} ('{}'): derived \
        ...
        for (ch_idx, (pixel_type, ch_name)) in channel_types.iter().enumerate() {
            ...
            if p + need > sample_bytes.len() {
                return Err(ExrError::invalid(format!(
                    "multi-part deep tiled MIPMAP part {part_idx} ('{}'): channel '{ch_name}' \
        ...
        if p != sample_bytes.len() {
            return Err(ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP part {part_idx} ('{}'): consumed {p} of {} payload bytes",
                ps.name,
                sample_bytes.len()
            )));
        }
        ...
        if ps.level_tiles[lvl][grid_idx].is_some() {
            return Err(ExrError::invalid(format!(
                "multi-part deep tiled MIPMAP part {part_idx} ('{}'): tile (lvl={lvl}, \
                 tx={tx_u}, ty={ty_u}) appears more than once",
                ps.name
            )));
        }
        ps.level_tiles[lvl][grid_idx] = Some(TileDecoded {
            tw,
            th,
            channel_samples: per_channel,
        });
    ...
    // Second pass: per part, per level, reassemble pixel-scan order.
    let mut out_parts: Vec<DeepMipmapTiledPart> = Vec::with_capacity(state.len());
    for ps in state {
        let mut levels_out: Vec<DeepTiledMipmapLevel> = Vec::with_capacity(ps.n_levels as usize);
        for (lvl, meta) in ps.metas.iter().enumerate() {
            for (idx, slot) in ps.level_tiles[lvl].iter().enumerate() {
                if slot.is_none() {
                    return Err(ExrError::invalid(format!(
                        "multi-part deep tiled MIPMAP part '{}': level {lvl} tile grid \
                         missing entry {idx}",
                        ps.name
                    )));
                }
            }
            let spp = &ps.level_spp[lvl];
            let total_samples: u64 = spp.iter().map(|&n| n as u64).sum();
            let mut channel_samples: Vec<Vec<f32>> = (0..ps.channels.len())
                .map(|_| Vec::with_capacity(total_samples as usize))
                .collect();
            let mut tile_pixel_starts: Vec<Vec<u64>> =
                Vec::with_capacity(ps.level_tiles[lvl].len());
            for (g_idx, slot) in ps.level_tiles[lvl].iter().enumerate() {
                let td = slot.as_ref().unwrap();
                let x0 = ((g_idx as u32) % meta.tx_count) * ps.tile_x;
                let y0 = ((g_idx as u32) / meta.tx_count) * ps.tile_y;
                let mut starts = Vec::with_capacity((td.tw * td.th) as usize + 1);
                starts.push(0u64);
                let mut acc: u64 = 0;
                for r in 0..td.th as usize {
            ...
            for y in 0..meta.height as usize {
                ...
                for x in 0..meta.width as usize {
                    ...
                    let td = ps.level_tiles[lvl][grid_idx].as_ref().unwrap();
                    let pixel_within_tile = y_in_tile * td.tw as usize + x_in_tile;
            ...
            levels_out.push(DeepTiledMipmapLevel {
                width: meta.width,
                height: meta.height,
                samples_per_pixel: spp.clone(),
                channel_samples,
            });
        }

        out_parts.push(DeepMipmapTiledPart {
            name: ps.name,
            data_window: ps.data_window,
            display_window: ps.display_window,
            line_order: ps.line_order,
...
mod tests {
    ...
    /// Build a `(nx × ny)` deep ripmap grid of synthetic deep pixels.
    /// Each grid cell carries an independent `synthetic_deep(cell_w,
    /// cell_h)` payload — the round-130 deterministic per-pixel sample
    /// counts (0..3 mod 4) and per-channel synthetic samples. This is
    /// not a content-coherent reduction across cells (RIPMAP does not
    /// constrain inter-cell sample relationships at the file-format
    /// level), it just exercises the per-cell encode/decode plumbing.
    #[allow(clippy::type_complexity)]
    fn synthetic_deep_ripmap(w0: u32, h0: u32) -> Vec<Vec<(Vec<u32>, [Vec<f32>; 4])>> {
        let nx = crate::decoder::mipmap_level_count(w0, false);
        let ny = crate::decoder::mipmap_level_count(h0, false);
        let mut grid = Vec::with_capacity(ny as usize);
        for lvly in 0..ny {
            let lh = crate::decoder::mipmap_level_dim(h0, lvly, false);
            let mut row = Vec::with_capacity(nx as usize);
            for lvlx in 0..nx {
                let lw = crate::decoder::mipmap_level_dim(w0, lvlx, false);
                row.push(synthetic_deep(lw, lh));
            }
            grid.push(row);
        }
        grid
    ...
    /// Build a [`DeepRipmapTiledInput`] from a synthetic ripmap grid
    /// produced by [`synthetic_deep_ripmap`].
    #[allow(clippy::type_complexity)]
    fn build_deep_ripmap<'a>(
        w0: u32,
        h0: u32,
    ...
    #[allow(clippy::type_complexity)]
    fn assert_deep_ripmap_roundtrip(
        img: &DeepRipmapTiledImage,
        grid: &[Vec<(Vec<u32>, [Vec<f32>; 4])>],
    ) {
        let (nx, ny) = img.level_counts();
        assert_eq!(ny as usize, grid.len(), "y-level count mismatch");
        assert_eq!(nx as usize, grid[0].len(), "x-level count mismatch");
        for (lvly, row) in grid.iter().enumerate() {
            for (lvlx, (spp, planes)) in row.iter().enumerate() {
                ...
                assert_eq!(
                    ...
                    "cell (lvlx={lvlx}, lvly={lvly}) spp mismatch"
                );
                for (ch_idx, plane) in planes.iter().enumerate() {
                    assert_eq!(
                        &cell.channel_samples[ch_idx], plane,
                        "cell (lvlx={lvlx}, lvly={lvly}) ch {ch_idx} sample mismatch"
                    );
                }
            }
        }
    ...
    #[test]
    fn deep_ripmap_tiled_roundtrip_none_16x16_tile_8() {
        // 16×16 → (nx, ny) = (5, 5). 25 grid cells; 8×8 tiles → cell
        // (0,0) has 4 tiles, the rest have 1 tile each.
        let (w0, h0) = (16u32, 16u32);
        let grid = synthetic_deep_ripmap(w0, h0);
        let input = build_deep_ripmap(w0, h0, &grid, 8, 8, Compression::None);
        ...
        assert_eq!(img.tile_x, 8);
        assert_eq!(img.tile_y, 8);
        assert_deep_ripmap_roundtrip(&img, &grid);
    }

    #[test]
    fn deep_ripmap_tiled_roundtrip_zips_24x16_tile_8x4() {
        // 24×16 → (nx, ny) = (5, 5). Edge tiles at many cells.
        let (w0, h0) = (24u32, 16u32);
        let grid = synthetic_deep_ripmap(w0, h0);
        let input = build_deep_ripmap(w0, h0, &grid, 8, 4, Compression::Zips);
        let bytes = encode_exr_deep_tiled_ripmap(&input).unwrap();
        let img = parse_exr_deep_tiled_ripmap(&bytes).unwrap();
        assert_deep_ripmap_roundtrip(&img, &grid);
    }
    ...
    #[test]
    fn deep_ripmap_tiled_roundtrip_rle_edge_tiles_13x9_tile_4() {
        // 13×9 → (nx, ny) = (4, 4). 4×4 tiles → cell (0,0) has 4×3=12
        // tiles with edge tiles on right and bottom; smaller cells
        // collapse but still exercise multi-axis edge tiles.
        let (w0, h0) = (13u32, 9u32);
        let grid = synthetic_deep_ripmap(w0, h0);
        let input = build_deep_ripmap(w0, h0, &grid, 4, 4, Compression::Rle);
        let bytes = encode_exr_deep_tiled_ripmap(&input).unwrap();
        let img = parse_exr_deep_tiled_ripmap(&bytes).unwrap();
        assert_deep_ripmap_roundtrip(&img, &grid);
    }
    ...
    #[test]
    fn deep_ripmap_tiled_rejects_zip_compression() {
        // ZIP rejected by the reference exrinfo for deep data — match.
        let (w0, h0) = (8u32, 8u32);
        let grid = synthetic_deep_ripmap(w0, h0);
        let input = build_deep_ripmap(w0, h0, &grid, 4, 4, Compression::Zip);
        assert!(encode_exr_deep_tiled_ripmap(&input).is_err());
    }

    #[test]
    fn deep_ripmap_tiled_rejects_wrong_y_level_count() {
        let (w0, h0) = (16u32, 16u32);
        let mut grid = synthetic_deep_ripmap(w0, h0);
        // 16×16 → 5 y-levels; truncate to 3.
        grid.truncate(3);
        let input = build_deep_ripmap(w0, h0, &grid, 8, 8, Compression::Zips);
        assert!(encode_exr_deep_tiled_ripmap(&input).is_err());
    }

    #[test]
    fn deep_ripmap_tiled_rejects_wrong_x_level_count() {
        let (w0, h0) = (16u32, 16u32);
        let mut grid = synthetic_deep_ripmap(w0, h0);
    ...
    #[test]
    fn deep_ripmap_tiled_rejects_subsampled_channels() {
        let (w0, h0) = (8u32, 8u32);
        let grid = synthetic_deep_ripmap(w0, h0);
        let mut input = build_deep_ripmap(w0, h0, &grid, 4, 4, Compression::Zips);
        input.channels = vec![Channel {
            name: "Y".to_string(),
            pixel_type: PixelType::Float,
            p_linear: false,
    ...
    #[test]
    fn deep_ripmap_tiled_rejects_zero_tile_size() {
        let (w0, h0) = (8u32, 8u32);
        let grid = synthetic_deep_ripmap(w0, h0);
        let input = build_deep_ripmap(w0, h0, &grid, 0, 4, Compression::Zips);
        assert!(encode_exr_deep_tiled_ripmap(&input).is_err());
    }

    #[test]
    fn deep_ripmap_tiled_rejects_empty_grid() {
    ...
    #[test]
    fn parse_exr_deep_tiled_redirects_ripmap_to_new_entry() {
        // Encode a RIPMAP deep tiled file, then assert that the legacy
        // single-part deep tiled reader (which only accepts ONE_LEVEL)
        // surfaces a pointer error referring to the new entry rather
        // than mis-parsing it.
        let (w0, h0) = (8u32, 8u32);
        let grid = synthetic_deep_ripmap(w0, h0);
        let input = build_deep_ripmap(w0, h0, &grid, 4, 4, Compression::Zips);
        let bytes = encode_exr_deep_tiled_ripmap(&input).unwrap();
        let err = parse_exr_deep_tiled(&bytes).unwrap_err();
        let msg = format!("{err}");
        assert!(
            msg.contains("parse_exr_deep_tiled_ripmap"),
            "expected pointer to parse_exr_deep_tiled_ripmap, got: {msg}"
        );
    }

    #[test]
    fn parse_exr_deep_tiled_mipmap_redirects_ripmap_pointer() {
        // Symmetric: encode a RIPMAP deep tiled file, then assert that
        // the MIPMAP reader rejects it explicitly with a pointer to the
        // new entry rather than mis-parsing.
        let (w0, h0) = (8u32, 8u32);
        let grid = synthetic_deep_ripmap(w0, h0);
        let input = build_deep_ripmap(w0, h0, &grid, 4, 4, Compression::Zips);
        let bytes = encode_exr_deep_tiled_ripmap(&input).unwrap();
        ...
        let msg = format!("{err}");
        assert!(
            msg.contains("parse_exr_deep_tiled_ripmap") || msg.contains("mode=0x02"),
            "expected RIPMAP rejection / pointer, got: {msg}"
        );
    }

    #[test]
    fn parse_exr_deep_tiled_ripmap_rejects_one_level_file() {
        // Symmetric guard: encode a single-part ONE_LEVEL deep tiled
        // file and verify the RIPMAP reader rejects it explicitly rather
        // than mis-parsing.
        let (spp, planes) = synthetic_deep(8, 8);
        let one_level = DeepTiledInput {
            width: 8,
        ...
        let bytes = encode_exr_deep_tiled(&one_level).unwrap();
        let err = parse_exr_deep_tiled_ripmap(&bytes).unwrap_err();
        let msg = format!("{err}");
        assert!(
            msg.contains("mode=0x00") || msg.contains("ONE_LEVEL"),
            "expected ONE_LEVEL rejection, got: {msg}"
    ...
    #[test]
    fn parse_exr_deep_tiled_ripmap_rejects_mipmap_file() {
        // Symmetric: encode a MIPMAP_LEVELS deep tiled file and verify
        // the RIPMAP reader rejects it explicitly.
        let (w0, h0) = (8u32, 8u32);
        let pyramid = synthetic_deep_mipmap(w0, h0);
        let input = build_deep_mipmap(w0, h0, &pyramid, 4, 4, Compression::Zips);
        let bytes = encode_exr_deep_tiled_mipmap(&input).unwrap();
        ...
        let msg = format!("{err}");
        assert!(
            msg.contains("mode=0x01") || msg.contains("MIPMAP"),
            "expected MIPMAP rejection, got: {msg}"
        );
    }

    #[test]
    fn deep_ripmap_tiled_non_power_of_two_16x12_tile_4() {
        // 16×12 → (nx, ny) = (5, 5). ROUND_DOWN halves each dim
        // independently; cell (lvlx, lvly) has dims
        // (mipmap_level_dim(16, lvlx), mipmap_level_dim(12, lvly)).
        let (w0, h0) = (16u32, 12u32);
        let grid = synthetic_deep_ripmap(w0, h0);
        let input = build_deep_ripmap(w0, h0, &grid, 4, 4, Compression::Zips);
        let bytes = encode_exr_deep_tiled_ripmap(&input).unwrap();
        let img = parse_exr_deep_tiled_ripmap(&bytes).unwrap();
        let (nx_got, ny_got) = img.level_counts();
        assert_eq!(nx_got, crate::decoder::mipmap_level_count(w0, false));
        assert_eq!(ny_got, crate::decoder::mipmap_level_count(h0, false));
        assert_deep_ripmap_roundtrip(&img, &grid);
    }

    #[test]
    fn deep_ripmap_tiled_version_field_bits() {
        // Single-part deep RIPMAP tiled discipline: only the non_image
        // (0x800) bit is set; multipart (0x1000) MUST NOT be set, and
        // single_tile (0x200) MUST NOT be set (mirroring the round-130
        // ONE_LEVEL and round-208 MIPMAP single-part deep tiled writers).
        let (w0, h0) = (8u32, 8u32);
        let grid = synthetic_deep_ripmap(w0, h0);
        let input = build_deep_ripmap(w0, h0, &grid, 4, 4, Compression::None);
        let bytes = encode_exr_deep_tiled_ripmap(&input).unwrap();
        let version = u32::from_le_bytes(bytes[4..8].try_into().unwrap());
        let bits = version & 0xFFFFFF00;
        assert_eq!(
    ...
    // -----------------------------------------------------------------
    // Round 220: multi-part deep tiled MIPMAP_LEVELS tests.
    // -----------------------------------------------------------------

    fn build_mp_deep_mipmap_part<'a>(
        name: &str,
        w0: u32,
        h0: u32,
        levels: &'a [(Vec<u32>, [Vec<f32>; 4])],
        ...
        tile_y: u32,
        compression: Compression,
    ) -> MultipartDeepMipmapTiledPart<'a> {
        let mut pyramid: Vec<DeepMipmapTiledLevelInput<'a>> = Vec::with_capacity(levels.len());
        for (l, (spp, planes)) in levels.iter().enumerate() {
            let lw = crate::decoder::mipmap_level_dim(w0, l as u32, false);
            let lh = crate::decoder::mipmap_level_dim(h0, l as u32, false);
    ...
    fn assert_mp_deep_mipmap_part_roundtrip(
        part: &DeepMipmapTiledPart,
        expected_name: &str,
        levels: &[(Vec<u32>, [Vec<f32>; 4])],
    ) {
        ...
        assert_eq!(
            part.level_count(),
            levels.len(),
            "level count for part {expected_name}"
        );
        for (l, (spp, planes)) in levels.iter().enumerate() {
            let lvl = &part.levels[l];
            assert_eq!(
                &lvl.samples_per_pixel, spp,
                "part {expected_name} level {l} spp"
            );
            for (ch_idx, plane) in planes.iter().enumerate() {
                assert_eq!(
                    &lvl.channel_samples[ch_idx], plane,
                    "part {expected_name} level {l} ch {ch_idx}"
                );
            }
        }
    }
    ...
    #[test]
    fn mp_deep_mipmap_tiled_two_parts_zips_roundtrip() {
        let (w0, h0) = (16u32, 16u32);
        let pyr_a = synthetic_deep_mipmap(w0, h0);
        let pyr_b = synthetic_deep_mipmap(w0, h0);
        let parts = vec![
            build_mp_deep_mipmap_part("alpha", w0, h0, &pyr_a, 8, 8, Compression::Zips),
            build_mp_deep_mipmap_part("beta", w0, h0, &pyr_b, 8, 8, Compression::Zips),
        ];
        let bytes = encode_exr_multipart_deep_tiled_mipmap(&parts).unwrap();
        let read = parse_exr_multipart_deep_tiled_mipmap(&bytes).unwrap();
        assert_eq!(read.len(), 2);
        assert_mp_deep_mipmap_part_roundtrip(&read[0], "alpha", &pyr_a);
        assert_mp_deep_mipmap_part_roundtrip(&read[1], "beta", &pyr_b);
        // Spot-check level 0 reported dims.
    ...
    #[test]
    fn mp_deep_mipmap_tiled_three_parts_mixed_compression_roundtrip() {
        let (w0, h0) = (16u32, 16u32);
        let pyr_a = synthetic_deep_mipmap(w0, h0);
        let pyr_b = synthetic_deep_mipmap(w0, h0);
        let pyr_c = synthetic_deep_mipmap(w0, h0);
        let parts = vec![
            build_mp_deep_mipmap_part("none_part", w0, h0, &pyr_a, 8, 8, Compression::None),
            build_mp_deep_mipmap_part("rle_part", w0, h0, &pyr_b, 8, 8, Compression::Rle),
            build_mp_deep_mipmap_part("zips_part", w0, h0, &pyr_c, 8, 8, Compression::Zips),
        ];
        let bytes = encode_exr_multipart_deep_tiled_mipmap(&parts).unwrap();
        let read = parse_exr_multipart_deep_tiled_mipmap(&bytes).unwrap();
        assert_eq!(read.len(), 3);
        assert_mp_deep_mipmap_part_roundtrip(&read[0], "none_part", &pyr_a);
        assert_mp_deep_mipmap_part_roundtrip(&read[1], "rle_part", &pyr_b);
        assert_mp_deep_mipmap_part_roundtrip(&read[2], "zips_part", &pyr_c);
        assert_eq!(read[0].compression, Compression::None);
        assert_eq!(read[1].compression, Compression::Rle);
        assert_eq!(read[2].compression, Compression::Zips);
    }
    ...
    #[test]
    fn mp_deep_mipmap_tiled_edge_tiles_non_power_of_two_roundtrip() {
        // 13×9 → 4 levels (13, 6, 3, 1). 4×4 tiles exercise edge tiles
        // throughout the pyramid; mix RLE + ZIPS across two parts.
        let (w0, h0) = (13u32, 9u32);
        let pyr_a = synthetic_deep_mipmap(w0, h0);
        let pyr_b = synthetic_deep_mipmap(w0, h0);
        let parts = vec![
            build_mp_deep_mipmap_part("p0", w0, h0, &pyr_a, 4, 4, Compression::Rle),
            build_mp_deep_mipmap_part("p1", w0, h0, &pyr_b, 4, 4, Compression::Zips),
        ];
        let bytes = encode_exr_multipart_deep_tiled_mipmap(&parts).unwrap();
        let read = parse_exr_multipart_deep_tiled_mipmap(&bytes).unwrap();
        assert_eq!(read.len(), 2);
        assert_mp_deep_mipmap_part_roundtrip(&read[0], "p0", &pyr_a);
        assert_mp_deep_mipmap_part_roundtrip(&read[1], "p1", &pyr_b);
    }

    #[test]
    fn mp_deep_mipmap_tiled_distinct_dims_per_part_roundtrip() {
        // Two parts of different level-0 dimensions exercises per-part
        // pyramid length / metas independence.
        let (w0_a, h0_a) = (16u32, 16u32);
        let (w0_b, h0_b) = (24u32, 16u32);
        let pyr_a = synthetic_deep_mipmap(w0_a, h0_a);
        let pyr_b = synthetic_deep_mipmap(w0_b, h0_b);
        let parts = vec![
            build_mp_deep_mipmap_part("small", w0_a, h0_a, &pyr_a, 8, 8, Compression::Zips),
            build_mp_deep_mipmap_part("wide", w0_b, h0_b, &pyr_b, 8, 4, Compression::None),
        ];
        let bytes = encode_exr_multipart_deep_tiled_mipmap(&parts).unwrap();
        let read = parse_exr_multipart_deep_tiled_mipmap(&bytes).unwrap();
        assert_eq!(read.len(), 2);
        assert_mp_deep_mipmap_part_roundtrip(&read[0], "small", &pyr_a);
        assert_mp_deep_mipmap_part_roundtrip(&read[1], "wide", &pyr_b);
        assert_eq!(read[0].width(), w0_a);
        assert_eq!(read[1].width(), w0_b);
    }

    #[test]
    fn mp_deep_mipmap_tiled_rejects_empty_parts() {
        let r = encode_exr_multipart_deep_tiled_mipmap(&[]);
        assert!(r.is_err());
    }

    #[test]
    fn mp_deep_mipmap_tiled_rejects_duplicate_names() {
        let (w0, h0) = (16u32, 16u32);
        let pyr = synthetic_deep_mipmap(w0, h0);
        let parts = vec![
            build_mp_deep_mipmap_part("dup", w0, h0, &pyr, 8, 8, Compression::Zips),
            build_mp_deep_mipmap_part("dup", w0, h0, &pyr, 8, 8, Compression::Zips),
        ];
        assert!(encode_exr_multipart_deep_tiled_mipmap(&parts).is_err());
    }

    #[test]
    fn mp_deep_mipmap_tiled_rejects_deep_zip() {
        let (w0, h0) = (16u32, 16u32);
        let pyr = synthetic_deep_mipmap(w0, h0);
    ...
    #[test]
    fn mp_deep_mipmap_tiled_rejects_wrong_pyramid_length() {
        let (w0, h0) = (16u32, 16u32);
        let full = synthetic_deep_mipmap(w0, h0);
        // Drop the last level → wrong count.
        let truncated: Vec<(Vec<u32>, [Vec<f32>; 4])> =
            full.iter().take(full.len() - 1).cloned().collect();
        let parts = vec![build_mp_deep_mipmap_part(
            "p",
            w0,
            h0,
    ...
    #[test]
    fn mp_deep_mipmap_tiled_parse_rejects_single_part_file() {
        // A single-part deep tiled MIPMAP file should be rejected.
        let (w0, h0) = (16u32, 16u32);
        let pyr = synthetic_deep_mipmap(w0, h0);
        let input = build_deep_mipmap(w0, h0, &pyr, 8, 8, Compression::Zips);
        let bytes = encode_exr_deep_tiled_mipmap(&input).unwrap();
    ...
    #[test]
    fn mp_deep_mipmap_tiled_parse_rejects_multipart_one_level_file() {
        // A multipart deep-tiled ONE_LEVEL file should be rejected by
        // the MIPMAP entry (mode=0x00 != 0x01).
        let (w, h) = (8u32, 8u32);
        ...
        let msg = format!("{:?}", r.unwrap_err());
        assert!(
            msg.contains("mode=0x00") || msg.contains("MIPMAP"),
            "got: {msg}"
        );
    }

    #[test]
    fn mp_deep_tiled_one_level_parser_redirects_mipmap_file() {
        // Conversely, the ONE_LEVEL multipart parser should redirect
        // (reject with a hint) when given a MIPMAP file.
        let (w0, h0) = (16u32, 16u32);
        let pyr = synthetic_deep_mipmap(w0, h0);
        let parts = vec![build_mp_deep_mipmap_part(
            "p",
        ...
        let msg = format!("{:?}", r.unwrap_err());
        assert!(
            msg.contains("parse_exr_multipart_deep_tiled_mipmap"),
            "got: {msg}"
        );
    }

    #[test]
    fn single_part_deep_mipmap_parser_redirects_multipart_mipmap_file() {
        // The single-part deep-MIPMAP parser should redirect when given
        // a multi-part deep-MIPMAP file.
        let (w0, h0) = (16u32, 16u32);
        let pyr = synthetic_deep_mipmap(w0, h0);
        let parts = vec![build_mp_deep_mipmap_part(
            "p",
        ...
        let bytes = encode_exr_multipart_deep_tiled_mipmap(&parts).unwrap();
        let r = parse_exr_deep_tiled_mipmap(&bytes);
        assert!(r.is_err());
        let msg = format!("{:?}", r.unwrap_err());
        assert!(
            msg.contains("parse_exr_multipart_deep_tiled_mipmap"),
    ...
    #[test]
    fn mp_deep_mipmap_tiled_version_field_bits() {
        let (w0, h0) = (16u32, 16u32);
        let pyr = synthetic_deep_mipmap(w0, h0);
        let parts = vec![build_mp_deep_mipmap_part(
            "p",
