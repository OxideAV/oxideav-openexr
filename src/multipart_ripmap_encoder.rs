...
pub fn encode_exr_multipart_tiled_ripmap(parts: &[MultipartRipmapTiledPart]) -> Result<Vec<u8>> {
    ...
    let disp_w = parts
        .iter()
        .map(|p| p.pyramid.grid[0][0].width)
        .max()
        .unwrap();
    let disp_h = parts
        .iter()
        .map(|p| p.pyramid.grid[0][0].height)
        .max()
        .unwrap();
    ...
    for (i, p) in parts.iter().enumerate() {
        let attrs = build_ripmap_tiled_part_attrs(p, chunk_counts[i] as i32, display_window);
...
mod tests {
    ...
    #[test]
    fn ripmap_multipart_accepts_every_compression_scheme() {
