...
| Attribute table                     | parse + write, eight required attributes typed plus typed inspectors for `int` / `double` / `string` / `v2i` / `v3i` / `v3f` / `m33f` / `m44f` / `chromaticities` / `box2f` (round-trip + validated by `exrheader`) |
