...
- Round-439 **DWAA / DWAB across the full flat surface**: single-part
  ...
  multi-part flat parts — decode and encode via the shared chunk codec
  (a tile is one chunk). With PIZ this closes the compression matrix:
  every flat writer/reader now accepts all ten compression codes (deep
  parts stay NONE / ZIPS / RLE by design). Round-trip tests across
  every surface plus reference cross-checks on the tiled path in both
  directions (`tests/dwa_tiled_multipart_validation.rs`). Also repairs
  a within-round `--no-default-features` breakage where the new `piz`
  module declaration had briefly captured the `registry` cfg gate.
